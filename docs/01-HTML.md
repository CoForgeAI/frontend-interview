...
|------|----------|-----------|
| **从属关系** | HTML 标签 | CSS 语法 |
| **加载时机** | 页面加载时同时加载 | 页面加载完成后加载 |
| **兼容性** | 无兼容问题 | CSS2.1+，IE5+ |
| **DOM 操作** | 可通过 JS 动态插入 | 无法通过 JS 操作 |
| **权重** | 无区别 | 无区别 |
...
**为什么推荐使用 link：**
1. **性能更好**：link 并行加载，@import 串行加载
2. **可控性强**：可以使用 media 属性按条件加载
3. **可操作性**：可以通过 JS 动态操作
...
### 17. img 标签的 alt 属性有什么作用？对 SEO 有什么影响？
...
1. **图片无法显示时**：显示替代文本
2. **无障碍访问**：屏幕阅读器会朗读 alt 文本
3. **SEO 优化**：帮助搜索引擎理解图片内容

```html
<!-- 好的示例 -->
<img src="cat.jpg" alt="一只橘色的猫咪躺在沙发上">
